...
uint32_t myl1pref::get_aht_index(uint64_t pc) const { return pc & (AHT_NUM_ENTRIES - 1); }
uint16_t myl1pref::get_aht_tag(uint64_t pc) const { return (pc >> AHT_TAG_INITIAL_SHIFT) & 0xFFFF; }
uint32_t myl1pref::get_pht_tdc_index(const std::array<int16_t, AHT_DELTA_HISTORY_SIZE>& delta_hist) const {
    uint32_t hash = 1984; 
...
uint64_t myl1pref::get_src_region_address(uint64_t block_addr) const { return block_addr >> SRC_LINES_PER_REGION_LOG2; }
uint8_t myl1pref::get_src_offset_in_region(uint64_t block_addr) const { return static_cast<uint8_t>(block_addr & SRC_REGION_MASK); }
uint32_t myl1pref::get_src_set_index(uint64_t region_addr) const { return region_addr & (SRC_NUM_SETS - 1); }
uint64_t myl1pref::get_src_tag(uint64_t region_addr) const { return (region_addr >> SRC_INDEX_BITS); }
uint8_t myl1pref::find_src_victim(uint32_t set_idx) const { return static_cast<uint8_t>(src_lru_way[set_idx]); }
void myl1pref::update_src_lru(uint32_t set_idx, bool accessed_way) { if (SRC_NUM_WAYS == 2) src_lru_way[set_idx] = !accessed_way; }
        ...
        if (success) {
            ...
            switch (engine_id) {
                case PrefetchSourceEngine::NL: num_prefetches_issued_nl++;
                  break;
                case PrefetchSourceEngine::TDC: num_prefetches_issued_tdc++;
                  break;
                case PrefetchSourceEngine::SRC: num_prefetches_issued_src++;
                  break;
...
void myl1pref::determine_best_engine_for_exploit() {
    ...
    */
    ...
    printf("[%lu] EXPLORE phase ended. PQ Hit Scores: NL=%d, TDC=%d, SRC=%d. Selected for EXPLOIT: Engines %d%d%d.\n",
    ...
    if (useful_prefetch) {
        ...
        if (engine_id_val == PrefetchSourceEngine::NL ||
            ...
            switch (engine_id_val) {
                case PrefetchSourceEngine::NL: num_prefetches_useful_nl++;
                  break;
                case PrefetchSourceEngine::TDC: num_prefetches_useful_tdc++;
                  break;
                case PrefetchSourceEngine::SRC: num_prefetches_useful_src++;
                  break;
                default: break; 
            }
            ...
            if (current_delta != 0) {
                ...
                if (pht_entry.valid && pht_tag_match) {
                    if (pht_entry.predicted_next_delta == current_delta) {
                        if (pht_entry.confidence < TDC_PHT_CONFIDENCE_MAX)
